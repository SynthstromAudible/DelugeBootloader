...
void R_RSPI_Start (uint8_t channel)
{
    ...
    if (0 == rza_io_reg_read_8( &(RSPI(channel).SPSR.BYTE),
...
uint8_t R_RSPI1_SendReceiveBasic(uint8_t channel, uint8_t data) {
    ...
    while (0 == rza_io_reg_read_8( &(RSPI(channel).SPSR.BYTE), RSPIn_SPSR_SPRF_SHIFT, RSPIn_SPSR_SPRF));
...
void R_RSPI_SendAndDontWait(uint8_t channel, uint8_t data) {
	...
	while (rza_io_reg_read_8( &(RSPI(channel).SPSR.BYTE),
                               RSPIn_SPSR_SPRF_SHIFT,
    		                   RSPIn_SPSR_SPRF)) {
