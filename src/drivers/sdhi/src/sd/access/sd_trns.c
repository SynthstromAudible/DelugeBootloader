 ...
 * Include      : 
 * Declaration  : int _sd_software_trans(SDHNDL *hndl,unsigned char *buff,
 *              : 	long cnt,int dir)
...
int _sd_software_trans(SDHNDL *hndl,unsigned char *buff,long cnt,int dir)
{
	...
	for(j=cnt; j>0 ;j--){
		...
		if((*func)(hndl->sd_port, buff,(unsigned long)hndl->reg_base+SD_BUF0,512) != SD_OK){
 ...
 * Include      : 
 * Declaration  : int _sd_dma_trans(SDHNDL *hndl,long cnt)
...
int _sd_dma_trans(SDHNDL *hndl,long cnt)
{
	...
	if(sddev_wait_dma_end(hndl->sd_port, cnt*512) != SD_OK){
