...
int sd_read_sect(int sd_port, unsigned char *buff,unsigned long psn,long cnt)
{
	...
	for(i=cnt; i > 0 ;
		...
		if(mode == SD_MODE_SW){	/* ==== PIO ==== */
			...
			#if		(TARGET_RZ_A1 == 1)
			if( dma_64 == SD_MODE_DMA_64 ){
				if(sddev_init_dma(sd_port, (unsigned long)buff,hndl->reg_base,		/* SD_CMD Address for 64byte transfer */
				...
				if(sddev_init_dma(sd_port, (unsigned long)buff,hndl->reg_base+SD_BUF0,
							cnt*512,SD_TRANS_READ) != SD_OK){
					_sd_set_err(hndl,SD_ERR_CPU_IF);
					goto ErrExit;
				}
	...
	int mode)
{
	...
	if(mode == SD_MODE_SW){	/* ==== PIO ==== */
		...
		if( dma_64 == SD_MODE_DMA_64 ){
			...
			if(sddev_init_dma(hndl->sd_port, (unsigned long)buff,hndl->reg_base,512,SD_TRANS_READ) != SD_OK){
				_sd_set_err(hndl,SD_ERR_CPU_IF);
				goto ErrExit;
			}
		}
		else{
			if(sddev_init_dma(hndl->sd_port, (unsigned long)buff,hndl->reg_base+SD_BUF0,512,SD_TRANS_READ) != SD_OK){
				_sd_set_err(hndl,SD_ERR_CPU_IF);
				goto ErrExit;
			}
		}
